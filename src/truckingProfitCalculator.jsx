...
const STORAGE_KEY = "trucking_profit_calculator_state_v2";
...
export default function TruckingProfitCalculator({
  ...
  const results = useMemo(() => {
    ...
    const numDaysPerWeek = Math.max(safeNum(daysPerWeek), 1);
    ...
    const deadheadFuelDaily = numDeadheadMilesPerDay * fuelCostPerMile;
    const fuelDaily = totalMilesPerDay * fuelCostPerMile;

    const loadedFuelProjected = projectedLoadedMiles * fuelCostPerMile;
    const deadheadFuelProjected = projectedDeadheadMiles * fuelCostPerMile;
    const fuelProjected = projectedTotalMiles * fuelCostPerMile;

    const maintenanceDaily = totalMilesPerDay * maintenanceCostPerMile;
    const maintenanceProjected = projectedTotalMiles * maintenanceCostPerMile;

    const tiresDaily = totalMilesPerDay * tireCostPerMile;
    const tiresProjected = projectedTotalMiles * tireCostPerMile;

    const miscDaily = totalMilesPerDay * miscCostPerMile;
    const miscProjected = projectedTotalMiles * miscCostPerMile;

    const variableDaily = totalMilesPerDay * variableCostPerMile;
    const variableProjected = projectedTotalMiles * variableCostPerMile;
    ...
    const dailyNetToTruck = splitRevenueDaily - totalDailyCost;
    const projectedNetToTruck = splitRevenueProjected - totalProjectedCost;

    const dailyNetPerDriver = mode === "team" ? dailyNetToTruck / 2 : dailyNetToTruck;
    const projectedNetPerDriver =
      mode === "team" ? projectedNetToTruck / 2 : projectedNetToTruck;
    ...
    const breakEvenRateLoadedMile =
      projectedLoadedMiles > 0
        ? totalProjectedCost /
          ...
          Math.max(numSplitPercent / 100, 0.01)
        : 0;
    ...
    const tierRows = tierPercents.map((pct) => {
      ...
      const tierDailyNetToTruck = dailyRevenueAfterSplit - totalDailyCost;
      const tierProjectedNetToTruck = projectedRevenueAfterSplit - totalProjectedCost;

      const tierDailyPerDriver =
        mode === "team" ? tierDailyNetToTruck / 2 : tierDailyNetToTruck;
      const tierProjectedPerDriver =
        mode === "team" ? tierProjectedNetToTruck / 2 : tierProjectedNetToTruck;
  ...
  const qualityCard =
    results.loadQuality === "bad"
      ? {
          tone: "red",
          title: "Bad load",
          body: "This setup is losing money after real operating costs. Do not trust the gross number.",
        }
      : results.loadQuality === "weak"
      ? {
          tone: "amber",
          title: "Weak load",
          body: "This load may keep the truck moving, but the after-tax money is thin.",
        }
      : results.loadQuality === "mid"
      ? {
          tone: "amber",
          title: "Decent load",
          body: "This is workable, but deadhead, split, or overhead can still drag it down fast.",
        }
      : {
          tone: "green",
          title: "Strong load",
          body: "This setup leaves real room after split, operating costs, and tax view.",
        };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-sm font-bold tracking-tight text-slate-900">
              Trucking Profit Calculator V2
            </div>
            <div className="text-xs text-slate-500">
              {profile?.email || session?.user?.email || "Driver account"} ·{" "}
              {subscription?.plan_name || (isTrial ? "trial" : "active")}
            </div>
          </div>
          ...
          <div className="mx-auto max-w-sm space-y-3 px-1 sm:max-w-2xl sm:space-y-5 sm:px-0 xl:max-w-6xl">
            <div className="rounded-2xl bg-white p-4 shadow-md sm:p-5">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div className="max-w-3xl">
                  <h1 className="text-xl font-bold tracking-tight text-slate-900 sm:text-3xl">
                    Real trucking margin view
                  </h1>
                  <p className="mt-1 text-sm leading-6 text-slate-600">
                    This version scores loads using split, fuel, maintenance, tires, overhead,
                    reserve, fees, deadhead, and a tax view.
                  </p>
                </div>
              ...
              <div className="mt-4 grid gap-3 xl:grid-cols-4">
                ...
                <AlertCard tone="slate" title="After-tax driver view">
            ...
            <div className="grid gap-4 xl:grid-cols-2 xl:gap-5">
              <div className="rounded-2xl bg-white p-4 shadow-md sm:p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  ...
                  <InputField
                    ...
                    step="1"
                    helpText="Planning field only. Projection uses Projection days."
                  />
                  <InputField
                    label="Split percent (%)"
                    value={splitPercent}
              ...
              <div className="rounded-2xl bg-white p-4 shadow-md sm:p-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Fuel alone is not real trucking math.
                  </p>
                </div>
                ...
                <div className="mt-5">
                  <h3 className="text-base font-bold text-slate-900">Fixed costs</h3>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
              ...
              <StatCard
                ...
                sublabel={`Custom projection active`}
              />
              <StatCard
                label="Projected loaded miles"
                value={results.projectedLoadedMiles.toLocaleString()}
                sublabel={`Total miles: ${results.projectedTotalMiles.toLocaleString()}`}
              />
              <StatCard
              ...
              />
              <StatCard
                label="Projected after split"
                value={fmt(results.splitRevenueProjected)}
                sublabel={`Total-mile rate: ${fmt(results.effectiveRatePerTotalMile)}/mi`}
              />
              <StatCard
                label="Break-even rate"
                value={`${fmt(results.breakEvenRateLoadedMile)}/mi`}
              ...
              <div className="rounded-2xl bg-white p-4 shadow-md sm:p-5">
                ...
                <div className="mt-4 space-y-3 text-sm text-slate-700">
                  ...
                  <div className="flex items-center justify-between gap-4 border-t border-slate-200 pt-3 font-semibold text-slate-900">
                    <span>Daily net to {mode === "team" ? "truck" : "driver"}</span>
                    <span>{fmt(results.dailyNetToTruck)}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4 font-semibold text-slate-900">
                    <span>{mode === "team" ? "Daily per driver" : "Daily take-home"}</span>
                    <span>{fmt(results.dailyNetPerDriver)}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4 font-semibold text-slate-900">
                    <span>Daily after tax</span>
                    <span>{fmt(results.dailyAfterTaxPerDriver)}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="mt-4 grid gap-3">
                  <div className="rounded-2xl bg-slate-50 p-5">
                    <div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">
                      Projected net to {mode === "team" ? "truck" : "driver"}
                    </div>
                    <div className="mt-2 text-3xl font-bold text-slate-900">
                      {fmt(results.projectedNetToTruck)}
                    </div>
                  </div>
